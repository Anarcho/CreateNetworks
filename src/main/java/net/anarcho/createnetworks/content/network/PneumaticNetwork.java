...
public class PneumaticNetwork {
    ...
    public void addPipe(BlockPos pos) {
        pipes.add(pos);
        CreateNetworks.LOGGER.debug("Added pipe at {}", pos);
    }

    public void removePipe(BlockPos pos) {
        pipes.remove(pos);
        CreateNetworks.LOGGER.debug("Removed pipe at {}", pos);
    }

    public void addInjector(BlockPos pos, IItemHandler itemHandler) {
        injectors.put(pos, itemHandler);
        CreateNetworks.LOGGER.debug("Added injector at {}", pos);
    }

    public void removeInjector(BlockPos pos) {
        injectors.remove(pos);
        CreateNetworks.LOGGER.debug("Removed injector at {}", pos);
    }

    public void addEjector(BlockPos pos, IItemHandler itemHandler) {
        ejectors.put(pos, itemHandler);
        CreateNetworks.LOGGER.debug("Added ejector at {}", pos);
    }

    public void removeEjector(BlockPos pos) {
        ejectors.remove(pos);
        CreateNetworks.LOGGER.debug("Removed ejector at {}", pos);
    }

    public void updateConnections(Level level, BlockPos pos) {
        pipes.add(pos);
        CreateNetworks.LOGGER.debug("Updated connections for pipe at {}", pos);
    }
    ...
    public void merge(PneumaticNetwork other) {
        ...
        CreateNetworks.LOGGER.debug("Merged networks. New size: {} pipes, {} injectors, {} ejectors",
    ...
    public boolean isActive() {
        ...
        CreateNetworks.LOGGER.debug("Network active status: {}. Injectors: {}, Ejectors: {}, Pipes: {}",
    ...
    public void tick() {
        ...
        CreateNetworks.LOGGER.debug("Ticking network with {} injectors and {} ejectors", injectors.size(), ejectors.size());
    ...
    private void transferItems(IItemHandler injector, BlockPos injectorPos) {
        for (int slot = 0; slot < injector.getSlots(); slot++) {
            ...
            if (!extracted.isEmpty()) {
                CreateNetworks.LOGGER.debug("Attempting to extract {} items from injector at {}", extracted.getCount(), injectorPos);
                ...
                if (targetPos != null) {
                    ...
                    if (inserted > 0) {
                        ...
                        CreateNetworks.LOGGER.debug("Transferred {} items from injector at {} to ejector at {}", inserted, injectorPos, targetPos);
                    }
                }
            }
        }
    ...
    public void setPunchcardInstructions(BlockPos pos, PunchcardData punchcardData) {
        ...
        CreateNetworks.LOGGER.debug("Set punchcard instructions at {}", pos);
    }

    public void removePunchcardInstructions(BlockPos pos) {
        punchcardInstructions.remove(pos);
        CreateNetworks.LOGGER.debug("Removed punchcard instructions at {}", pos);
    }
